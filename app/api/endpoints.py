    ...
    if not recommendation_service.initialized:
        ...
        if rag_service.initialized:
            if not getattr(recommendation_service, "_initializing", False):
                ...
                asyncio.create_task(
                    recommendation_service.initialize(rag_service.documents)
                )
...
async def process_query(
    ...
    recommendation_svc: EnhancedRecommendationService = Depends(
    ...
    try:
        ...
        try:
            ...
            response, processing_time = await asyncio.wait_for(
                rag_svc.process_query(request.query, user_id=request.user_id),
                timeout=timeout,
            )
        except asyncio.TimeoutError:
            # If we timeout, create a basic response
            logger.warning(
                f"Query processing timed out after {timeout}s: {request.query}"
            )
        ...
        background_tasks.add_task(
            _mark_documents_as_viewed, request.user_id, source_ids
        )
...
async def health_check():
    ...
    health_data["recommendation_status"] = (
        "online" if recommendation_service.initialized else "initializing"
    )
    health_data["search_status"] = (
        "online" if search_engine.document_vectors is not None else "initializing"
    )
        ...
        try:
            ...
            health_data["metrics"] = {
                ...
                "recent_processing_times": (
                    rag_metrics["processing_times"][-5:]
                    if "processing_times" in rag_metrics
                    else []
                ),
            }
        except Exception as e:
    ...
    try:
        ...
        user_metrics = {
            ...
            "avg_queries_per_user": (
                rag_metrics["total_queries"] / max(1, len(recommendation_service.users))
            ),
    ...
    try:
        ...
        try:
            ...
            if updated:
                ...
                return {
                    "status": "success",
                    "message": "No new documents found to update",
                }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update knowledge base: {str(e)}"
        )
...
async def mark_document_viewed(document_id: str, request: Dict[str, Any]):
    ...
    return {
        "status": "success",
        "message": f"Document {document_id} marked as viewed by user {user_id}",
    }


@router.post("/evaluate/recommendations")
async def evaluate_recommendations(request: Dict[str, Any]):
    ...
    try:
        evaluation = await evaluate_recommendation_quality(
            query, recommendations, user_history
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to evaluate recommendations: {str(e)}"
        )
