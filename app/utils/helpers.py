...
def time_function(func: Callable) -> Callable:
    ...
    async def async_wrapper(*args, **kwargs) -> Any:
        ...
        try:
            ...
            if (
                func.__name__ == "process_query"
                and args
                and hasattr(args[0], "query_metrics")
            ):
                service = args[0]
            ...
            execution_time = time.time() - start_time
            logger.error(
                f"Exception in {func.__name__}: {e} (took {execution_time:.4f} seconds)"
            )
            raise
    ...
    while start < text_length:
        ...
        if end < text_length and end - start == chunk_size:
            ...
            for separator in separators[
                :2
            ]:  # Just check the profile separators first (## and ###)
            ...
            if not profile_separator_found:
                for separator in separators[2:]:  # Check other separators
                    ...
                    if (
                        last_separator > start + chunk_size / 2
                    ):  # Only use if reasonably far into chunk
...
def is_out_of_scope(
    query: str, threshold: float, relevance_scores: List[float]
) -> bool:
...
def _clean_cache(now: float, ttl_seconds: int):
    ...
    expired_keys = [
        k for k, (_, t) in _operation_cache.items() if now - t > ttl_seconds
    ]
