...
class SearchEngine:
    ...
    def _extract_skills_from_text(self, text: str) -> List[str]:
        ...
        skill_section_match = re.search(r'(?:skills?|expertise)[:\s]*(.*?)(?:\n\n|\n\*\*|\Z)', 
    ...
    def search_talent(self, query: str, limit: int = 3) -> List[Tuple[Document, float]]:
        ...
        talent_docs = [doc for doc in self.documents 
                     if 'freelancer-profiles' in doc.path or 'freelancers' in doc.path]
        talent_indices = [i for i, doc in enumerate(self.documents) 
                        if doc in talent_docs]
    ...
    def _enhance_talent_query(self, query: str) -> str:
        ...
        # Detect job roles
        job_roles = ["developer", "programmer", "engineer", "designer", "writer", 
                   "marketer", "consultant", "manager", "expert", "specialist"]
        ...
        if enhanced_terms:
            enhanced_query = f"{query} {' '.join(enhanced_terms)} {' '.join(enhanced_terms)}"
    ...
    def analyze_intent(self, query: str) -> Dict[str, Any]:
        ...
        talent_keywords = ["find", "looking for", "hire", "need", "search", "freelancer", 
                         "developer", "designer", "writer", "expert"]
        ...
        return {
            ...
            "enhanced_query": self._enhance_talent_query(query) if is_talent_search else query
        }