...
class EnhancedRecommendationService:
    ...
    def __init__(self):
        ...
        self.recommendation_metrics = {
            ...
            "diversity_scores": [],
            "relevance_scores": [],
            "user_satisfaction": defaultdict(
                list
            ),  # This creates an empty list for any new key automatically
        }
    ...
    async def _categorize_documents(self):
        ...
        for doc in self.documents:
            ...
            if "freelancer-profiles" in doc.path or "talent" in doc.path:
                ...
                "technical" in doc.path or "shakers-technical-documentation" in doc.path
            ):
                categories["technical"].append(doc.id)
                self.document_categories[doc.id] = "technical"
            elif (
                "guide" in doc.path
                or "how-to" in doc.path
                or "finding_talent" in doc.path
            ):
        ...
        logger.info(
            f"Categorized {len(self.documents)} documents into {len(categories)} categories"
        )
    ...
    async def _update_user_interests(
        self, user_id: str, message: str, sources_used: List[str] = None
    ):
    ...
    def _calculate_diversity_score(self, doc_ids: List[str]) -> float:
        ...
        # Count categories
        categories = [
            self.document_categories.get(doc_id, "general") for doc_id in doc_ids
        ]
        ...
        try:
            ...
            if len(recommendations) < settings.MAX_RECOMMENDATIONS:
                ...
                for rec in general_recs:
                    if not any(
                        r["document_id"] == rec["document_id"] for r in recommendations
                    ):
    ...
    async def _get_query_recommendations(
        self, query: str, viewed_docs: set, intent: Dict[str, Any], limit: int = 3
    ) -> List[Dict[str, Any]]:
    ...
    async def _get_interest_recommendations(
        self, user_id: str, current_query: str, viewed_docs: set, limit: int = 2
    ) -> List[Dict[str, Any]]:
        """Get recommendations based on user interests."""
        user_profile = self.users[user_id]
        recommendations = []
        ...
        sorted_interests = sorted(
            user_profile.interests, key=lambda x: x.strength, reverse=True
        )
                ...
                recommendations.append(
                    {
                        ...
                        "explanation": explanation,
                        "relevance_score": float(score)
                        * 0.9,  # Slightly lower than direct matches
                        "tags": doc.tags if hasattr(doc, "tags") and doc.tags else [],
                    }
                )
    ...
    async def _generate_fallback_recommendations(
        ...
        try:
            ...
            for category in categories:
                ...
                unviewed_docs = [
                    doc for doc in category_docs if doc.id not in viewed_docs
                ]
                ...
                if unviewed_docs:
                    ...
                    fallback_recs.append(
                        {
                            ...
                            "relevance_score": 0.5,
                            "tags": (
                                doc.tags if hasattr(doc, "tags") and doc.tags else []
                            ),
                        }
                    )
                    ...
                    candidates = [
                        ...
                        if not any(
                            rec["document_id"] == doc.id for rec in fallback_recs
                        )
                    ]
