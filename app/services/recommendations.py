...
from app.models.schemas import UserProfile, Query, Document, Recommendation, ChatMessage, UserInterest
...
class RecommendationService:
    ...
    async def initialize(self, documents: List[Document]):
        ...
        self.document_similarity = squareform(pdist(self.document_vectors.toarray(), 'cosine'))
    ...
    async def _update_user_interests(self, user_id: str, message: str):
        ...
        similarities = cosine_similarity(message_vector, self.document_vectors).flatten()
    ...
    async def generate_recommendations(self, user_id: str, current_query: str) -> List[Recommendation]:
        ...
        try:
            ...
            query_similarity = cosine_similarity(query_vector, self.document_vectors).flatten()
                ...
                for i, (doc_idx, score) in enumerate(document_scores):
                    ...
                    for term in tech_terms:
                        if term.lower() in doc_content:
                            ...
                            logger.debug(f"Boosted score for document {doc.title} containing term {term}")
            ...
            for doc_idx, similarity in document_scores:
                ...
                if doc_cluster is not None and doc_cluster in selected_clusters and len(selected_clusters) < 3:
                    logger.debug(f"Skipping document from already selected cluster: {doc_cluster}")
                    continue
    ...
    def _generate_enhanced_explanation(self, document: Document, query: str, similarity: float, user_profile: UserProfile) -> str:
        ...
        matching_interests = [
            ...
            if interest.document_id == document.id or interest.topic.lower() in document.title.lower()
        ]
        ...
        if similarity > 0.8:
            ...
            time_frame = "recent" if (datetime.now() - interest.last_mentioned).days < 3 else "previous"
