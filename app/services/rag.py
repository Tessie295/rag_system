...
class RAGService:
    ...
    async def initialize(self, force_refresh=False):
        ...
        if vector_store_exists and not force_refresh:
            try:
                ...
                logger.info(
                    f"Vector store loaded in {time.time() - start_time:.2f} seconds"
                )
                ...
                logger.info(
                    f"Documents loaded in {time.time() - start_time:.2f} seconds"
                )
        ...
        raw_documents = await loop.run_in_executor(
            self.executor, lambda: load_knowledge_base(settings.KNOWLEDGE_BASE_DIR)
        )
        ...
        self.last_update_timestamp = time.time()
        logger.info(
            f"RAG service initialized in {time.time() - start_time:.2f} seconds"
        )
    ...
    async def analyze_query(self, query: str) -> Dict[str, Any]:
        ...
        query_analysis = {
            ...
            "relevance_variance": (
                np.var(relevance_scores) if len(relevance_scores) > 1 else 0
            ),
        ...
        if is_skill_query or is_profile_query:
            ...
            query_analysis["is_out_of_scope"] = (
                max(relevance_scores) < adjusted_threshold if relevance_scores else True
            )
        else:
            query_analysis["is_out_of_scope"] = (
                max(relevance_scores) < settings.SIMILARITY_THRESHOLD
                if relevance_scores
                else True
            )
    ...
    async def process_query(self, query: str, user_id: str = None) -> RAGResponse:
        ...
        logger.debug(
            f"Document retrieval completed in {time.time() - retrieval_start:.4f}s"
        )
        ...
        if settings.ENABLE_EVALUATION and time.time() - start_time < 3.0:
            ...
            try:
                ...
                evaluation = await evaluate_answer(
                    query, answer, [doc.page_content for doc, _ in retrieval_results]
                )
                logger.debug(
                    f"Evaluation completed in {time.time() - evaluation_start:.4f}s"
                )
            except Exception as e:
    ...
    async def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "answered_ratio": self.query_metrics["answered_queries"]
            / max(1, self.query_metrics["total_queries"]),
            "out_of_scope_ratio": self.query_metrics["out_of_scope_queries"]
            / max(1, self.query_metrics["total_queries"]),
            "avg_processing_time": sum(self.query_metrics["processing_times"])
            / max(1, len(self.query_metrics["processing_times"])),
            "avg_relevance_score": sum(self.query_metrics["relevance_scores"])
            / max(1, len(self.query_metrics["relevance_scores"])),
            "document_count": len(self.documents),
            "last_update": self.last_update_timestamp,
            "processing_times": (
                self.query_metrics["processing_times"][-10:]
                if self.query_metrics["processing_times"]
                else []
            ),
        }
        return metrics
