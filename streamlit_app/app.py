...
with col2:
    st.markdown(
        "<div class='main-header'>Shakers AI Support System</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div class='info-text'>Find answers about the platform and discover the perfect talent for your projects</div>",
        unsafe_allow_html=True,
    )
...
with st.sidebar:
    ...
    with st.expander("Advanced Settings"):
        st.checkbox(
            "Enable detailed explanations", value=True, key="detailed_explanations"
        )
        ...
        if health_response.status_code == 200:
            ...
            if "metrics" in health_data:
                ...
                with metric_cols[1]:
                    ...
                    st.metric(
                        "Avg Response Time",
                        f"{metrics.get('avg_processing_time', 0):.2f}s",
                    )
                    st.markdown("</div>", unsafe_allow_html=True)
    ...
    if st.session_state.response_times:
        ...
        if len(st.session_state.response_times) > 1:
            ...
            fig.update_layout(
                height=250, margin=dict(l=10, r=10, t=30, b=10), hovermode="x unified"
            )
            ...
            avg_scores = {
                ...
                "Score": [
                    sum(
                        ...
                        for score in st.session_state.evaluation_scores
                    )
                    / len(st.session_state.evaluation_scores),
                    sum(
                    ...
                    with st.expander("📄 View Sources"):
                        for source in message["sources"]:
                            ...
                            relevance_text = (
                                ...
                                if st.session_state.show_relevance
                                else ""
                            )

                            st.markdown(
                                f"<div class='source-card'>", unsafe_allow_html=True
                            )
                            st.markdown(f"**{source['title']}**{relevance_text}")

                            if "snippet" in source and source["snippet"]:
                                st.markdown(
                                    f"<div class='info-text'>{source['snippet']}</div>",
                                    unsafe_allow_html=True,
                                )

                            # Add a button to mark as viewed explicitly
                            if st.button(
                                f"View Document: {source['title']}",
                                key=f"view_{uuid.uuid4()}_{source['document_id']}_{message['timestamp']}",
                            ):
                                if (
                                    source["document_id"]
                                    not in st.session_state.viewed_documents
                                ):
                                    ...
                                    try:
                                        ...
                                        st.session_state.viewed_documents.add(
                                            source["document_id"]
                                        )
                                        st.success(
                                            f"Marked '{source['title']}' as viewed"
                                        )
                                    except:
    ...
    with recommendation_container:
        ...
        if st.session_state.last_recommendations:
            try:
                for rec in st.session_state.last_recommendations:
                    ...
                    title = rec.get(
                        ...
                        (
                            ...
                            else getattr(rec, "title", "Unknown Title")
                        ),
                    )
                    doc_id = rec.get(
                        "document_id",
                        (
                            rec["document_id"]
                            if isinstance(rec, dict)
                            else getattr(rec, "document_id", "unknown")
                        ),
                    )
                    explanation = rec.get(
                        "explanation",
                        (
                            rec["explanation"]
                            if isinstance(rec, dict)
                            else getattr(rec, "explanation", "Recommended resource")
                        ),
                    )
                    ...
                    st.markdown(f"**{title}**")
                    st.markdown(
                        f"<div class='info-text'>{explanation}</div>",
                        unsafe_allow_html=True,
                    )
                    ...
                    try:
                        tags = rec.get(
                            ...
                            (
                                ...
                                else getattr(rec, "tags", [])
                            ),
                        )
                        if tags:
                            tags_html = " ".join(
                                [
                                    f"<span class='highlight'>{tag}</span>"
                                    for tag in tags
                                ]
                            )
                            st.markdown(
                ...
                logger.error(
                    f"Recommendation data: {st.session_state.last_recommendations}"
                )
        else:
            st.info(
                "Recommendations will appear based on your questions.\nTry asking about specific skills like 'Angular' or 'Python'."
            )
...
if query:
    ...
    st.session_state.chat_history.append(
        {"role": "user", "content": query, "timestamp": message_timestamp}
    )
            ...
            if "sources" in result and result["sources"]:
                with assistant_response.expander("📄 View Sources"):
                    for source in result["sources"]:
                        ...
                        if st.button(
                            f"View Document: {source['title']}",
                            key=f"view_{source['document_id']}_{message_timestamp}_{uuid.uuid4()}",
                        ):
                            if (
                                source["document_id"]
                                not in st.session_state.viewed_documents
