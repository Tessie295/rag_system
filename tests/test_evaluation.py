...
class TestEvaluation:
    ...
    async def test_evaluate_answer(self):
        ...
        with patch(
            "app.utils.evaluation.evaluate_factual_correctness"
        ) as mock_correctness, patch(
            "app.utils.evaluation.evaluate_relevance"
        ) as mock_relevance, patch(
            "app.utils.evaluation.evaluate_completeness"
    ...
    async def test_evaluate_factual_correctness_with_llm(self):
        ...
        with patch("app.utils.evaluation.settings") as mock_settings, patch(
            "app.utils.evaluation.ChatOpenAI"
        ) as mock_chatgpt, patch(
            "app.utils.evaluation.ChatPromptTemplate"
        ) as mock_template, patch(
            "app.utils.evaluation.LLMChain"
        ) as mock_chain:
    ...
    async def test_evaluate_factual_correctness_timeout(self):
        ...
        with patch("app.utils.evaluation.settings") as mock_settings, patch(
            ...
            "app.utils.evaluation.evaluate_lexical_correctness"
        ) as mock_lexical:
    ...
    def test_evaluate_lexical_correctness(self):
        ...
        score_diff = evaluate_lexical_correctness(
            "The weather is nice today.", ["Shakers provides a secure payment system."]
        )
    ...
    def test_evaluate_relevance(self):
        ...
        with patch("app.utils.evaluation.TfidfVectorizer") as mock_tfidf, patch(
            "app.utils.evaluation.cosine_similarity"
        ) as mock_cosine:
    ...
    def test_generate_evaluation_feedback(self):
        ...
        assert (
            "relevant" in mixed_feedback.lower()
            or "relevance" in mixed_feedback.lower()
        )
    ...
    async def test_evaluate_recommendation_quality(self):
        ...
        with patch("app.utils.evaluation.TfidfVectorizer") as mock_tfidf:
            mock_tfidf.side_effect = Exception("Simulated error")
            evaluation_error = await evaluate_recommendation_quality(
