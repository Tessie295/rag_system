...
class TestEnhancedRecommendations:
    ...
    def mock_search_engine(self):
        ...
        with patch('app.services.enhanced_recommendations.SearchEngine') as MockSearchEngine:
            mock_instance = MagicMock()
            mock_instance.initialize = MagicMock()
            mock_instance.search = MagicMock(return_value=[
                (Document(id="doc1", title="Test Doc", content="Test", path="/test", tags=["test"]), 0.8)
            ])
            mock_instance.tech_skills = ["react", "python", "java", "javascript"]
            mock_instance.analyze_intent = MagicMock(return_value={
    ...
    async def test_initialize(self, recommendation_service, mock_documents):
        ...
        # Mock dependencies
        with patch('app.services.enhanced_recommendations.load_metrics') as mock_load_metrics:
            ...
            with patch.object(recommendation_service, '_load_user_profiles') as mock_load_profiles, \
                 patch.object(recommendation_service, '_categorize_documents') as mock_categorize:
    ...
    async def test_load_user_profiles(self, recommendation_service):
        ...
        with patch('os.path.exists') as mock_exists, \
             patch('builtins.open', mock_open(read_data=json.dumps(sample_profiles))):
    ...
    async def test_add_chat_to_user_history(self, recommendation_service):
        ...
        with patch.object(recommendation_service, '_update_user_interests') as mock_update_interests, \
             patch.object(recommendation_service, '_save_user_profiles') as mock_save_profiles:
    ...
    async def test_update_user_interests(self, recommendation_service, mock_documents):
        ...
        # Mock extract_topics
        with patch.object(recommendation_service, '_extract_topics') as mock_extract_topics:
            ...
            await recommendation_service._update_user_interests(user_id, message, sources_used)
    ...
    def test_extract_topics(self, recommendation_service):
        ...
        payment_text = "How does the payment system work on Shakers? I'm curious about escrow."
    ...
    async def test_mark_document_as_viewed(self, recommendation_service):
        ...
        # Set up mocks
        with patch.object(recommendation_service, '_save_user_profiles') as mock_save_profiles, \
             patch('app.services.enhanced_recommendations.save_metrics') as mock_save_metrics:
            ...
            assert any(interest.document_id == document_id for interest in user.interests)
            
            # Check that metrics were updated
            assert recommendation_service.recommendation_metrics["user_interactions"] > 0
    ...
    def test_calculate_diversity_score(self, recommendation_service):
        ...
        same_category_score = recommendation_service._calculate_diversity_score(["doc1", "doc5"])
        assert same_category_score == 0.5  # 1 unique category / 2 documents
        
        # Test with diverse documents
        diverse_score = recommendation_service._calculate_diversity_score(["doc1", "doc2", "doc3"])
        assert diverse_score == 1.0  # 3 unique categories / 3 documents
        
        # Test with mix of diverse and duplicate categories
        mixed_score = recommendation_service._calculate_diversity_score(["doc1", "doc2", "doc3", "doc5"])
    ...
    async def test_generate_recommendations(self, recommendation_service):
        ...
        # Set up mocks
        with patch.object(recommendation_service, '_get_query_recommendations') as mock_query_recs, \
             patch.object(recommendation_service, '_get_interest_recommendations') as mock_interest_recs, \
             patch.object(recommendation_service, '_generate_fallback_recommendations') as mock_fallback_recs, \
             patch.object(recommendation_service, '_save_user_profiles') as mock_save_profiles, \
             patch('app.services.enhanced_recommendations.save_metrics') as mock_save_metrics:
            ...
            recommendations = await recommendation_service.generate_recommendations(user_id, query)
            ...
            # Check that metrics were updated
            assert recommendation_service.recommendation_metrics["total_recommendations"] > 0
            assert len(recommendation_service.recommendation_metrics["diversity_scores"]) > 0
            assert len(recommendation_service.recommendation_metrics["relevance_scores"]) > 0
            ...
            recommendations = await recommendation_service.generate_recommendations(new_user_id, query)
            ...
            empty_recs = await recommendation_service.generate_recommendations(user_id, query)
            assert empty_recs == []
            ...
            error_recs = await recommendation_service.generate_recommendations(user_id, query)
    ...
    async def test_get_query_recommendations(self, recommendation_service):
        ...
        talent_intent = {"is_talent_search": True, "skills_mentioned": ["react", "javascript"]}
        
        # Set up search results
        doc1 = Document(id="doc1", title="Doc 1", content="Content", path="/path1", tags=["test"])
        doc2 = Document(id="doc2", title="Doc 2", content="Content", path="/path2", tags=["test"])
        doc3 = Document(id="doc3", title="Doc 3", content="Content", path="/path3", tags=["test"])
        
        # Mock search methods
        with patch.object(recommendation_service.search_engine, 'search') as mock_search, \
             patch.object(recommendation_service.search_engine, 'search_talent') as mock_search_talent:
            ...
            mock_search_talent.return_value = [
                (doc1, 0.9),
                (doc2, 0.8)
            ]
            ...
            normal_recs = await recommendation_service._get_query_recommendations(query, viewed_docs, normal_intent)
            ...
            talent_recs = await recommendation_service._get_query_recommendations(query, viewed_docs, talent_intent)
            
            # Check results
            assert len(talent_recs) > 0
            assert talent_recs[0]["document_id"] == "doc1"
            assert "expertise in react, javascript" in talent_recs[0]["explanation"].lower() or \
                  "Profile with expertise" in talent_recs[0]["explanation"]
            assert not mock_search.called
            assert mock_search_talent.called
            
            # Test with already viewed document
            viewed_docs = {"doc1", "doc2"}
            limited_recs = await recommendation_service._get_query_recommendations(query, viewed_docs, normal_intent)
    ...
    async def test_get_interest_recommendations(self, recommendation_service):
        ...
        with patch.object(recommendation_service.search_engine, 'search') as mock_search:
            ...
            def mock_search_side_effect(query, limit):
                if "hiring" in query:
                    return [
                        (Document(id="doc1", title="Finding Talent", content="Content", path="/path1", tags=["hiring"]), 0.9)
                    ]
                elif "payments" in query:
                    return [
                        (Document(id="doc2", title="Payments", content="Content", path="/path2", tags=["payments"]), 0.8)
                    ]
                else:
                    return [
                        (Document(id="doc3", title="React Developers", content="Content", path="/path3", tags=["react"]), 0.7)
                    ]
            
            mock_search.side_effect = mock_search_side_effect
            
            # Test getting interest recommendations
            interest_recs = await recommendation_service._get_interest_recommendations(user_id, current_query, viewed_docs)
            ...
            for rec in interest_recs:
                assert "interests" in rec["explanation"].lower() or \
                       "topics" in rec["explanation"].lower()
            
            # Test with no matching interests
            empty_recs = await recommendation_service._get_interest_recommendations(user_id, "completely unrelated query", viewed_docs)
            assert len(empty_recs) <= len(interest_recs)
            
            # Test with no interests
            recommendation_service.users[user_id].interests = []
            no_interests_recs = await recommendation_service._get_interest_recommendations(user_id, current_query, viewed_docs)
    ...
    async def test_generate_fallback_recommendations(self, recommendation_service):
        ...
        recommendation_service.documents = [
            Document(id=f"doc{i}", title=f"Test Doc {i}", content=f"Content {i}", path=f"/path{i}")
            for i in range(1, 5)
        ]
        ...
        with patch('numpy.random.choice') as mock_random_choice:
            ...
            fallback_recs = await recommendation_service._generate_fallback_recommendations(user_id)
            
            # Test should pass if we get any recommendations
            assert isinstance(fallback_recs, list)
            
            # Test with limit
            limited_recs = await recommendation_service._generate_fallback_recommendations(user_id, limit=1)
            assert isinstance(limited_recs, list)
            assert len(limited_recs) <= max(1, len(recommendation_service.documents) - 1)
            
            # Test with new user
            new_user_id = "new_user"
            new_user_recs = await recommendation_service._generate_fallback_recommendations(new_user_id)
