...
class TestEndpoints:
    ...
    async def test_process_query(self):
        ...
        with patch('app.api.endpoints.get_rag_service', return_value=mock_rag_service), \
             patch('app.api.endpoints.get_recommendation_service', return_value=mock_recommendation_service):
    ...
    async def test_health_check(self):
        ...
        with patch('app.api.endpoints.rag_service') as mock_rag, \
             patch('app.api.endpoints.recommendation_service') as mock_rec, \
             patch('app.api.endpoints.search_engine') as mock_search:
    ...
    async def test_update_knowledge_base(self):
        ...
        with patch('app.api.endpoints.rag_service') as mock_rag, \
             patch('app.api.endpoints.get_rag_service') as mock_get_rag:
    ...
    async def test_evaluate_recommendations(self):
        ...
        with patch('app.api.endpoints.evaluate_recommendation_quality') as mock_evaluate:
            ...
            request = {
                "query": "test query",
                "recommendations": [
                    {"document_id": "rec1", "title": "Rec 1", "path": "/path1", "relevance_score": 0.9}
                ]
            }
            ...
            # Check error message
            assert "required" in str(excinfo.value).lower() or "query" in str(excinfo.value).lower()
    ...
    async def test_get_search_engine_dependency(self):
        ...
        with patch('app.api.endpoints.rag_service') as mock_rag, \
            ...
            # Should not have called initialize
            assert not hasattr(mock_search, 'initialize') or not mock_search.initialize.called
